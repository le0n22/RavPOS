const express = require('express');
const router = express.Router();
const knex = require('../db');
const jwt = require('jsonwebtoken');

function authenticateToken(req, res, next) {
  const authHeader = req.headers['authorization'];
  const token = authHeader && authHeader.split(' ')[1];
  if (!token) return res.status(401).json({ error: 'Token gerekli' });

  jwt.verify(token, process.env.JWT_SECRET || 'gizli_anahtar', (err, user) => {
    if (err) return res.status(403).json({ error: 'Geçersiz token' });
    req.user = user;
    next();
  });
}

// Tüm ödemeleri getir (korumalı)
router.get('/', authenticateToken, async (req, res) => {
  try {
    const result = await knex('payments').select('*').orderBy('id');
    const resultWithStringIds = result.map(row => ({ ...row, id: String(row.id), order_id: row.order_id !== undefined ? String(row.order_id) : undefined }));
    res.json(resultWithStringIds);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
});

// Yeni ödeme ekle (korumalı)
router.post('/', authenticateToken, async (req, res) => {
  const { order_id, amount, method, status } = req.body;
  try {
    const [inserted] = await knex('payments')
      .insert({ order_id, amount, method, status })
      .returning('*');
    const insertedWithStringId = { ...inserted, id: String(inserted.id) };
    res.status(201).json(insertedWithStringId);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
});

// Ödeme güncelle (korumalı)
router.put('/:id', authenticateToken, async (req, res) => {
  const { order_id, amount, method, status } = req.body;
  const { id } = req.params;
  try {
    const [updated] = await knex('payments')
      .where({ id })
      .update({ order_id, amount, method, status })
      .returning('*');
    const updatedWithStringId = { ...updated, id: String(updated.id) };
    res.json(updatedWithStringId);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
});

// Ödeme sil (korumalı)
router.delete('/:id', authenticateToken, async (req, res) => {
  const { id } = req.params;
  try {
    await knex('payments').where({ id }).del();
    res.json({ message: 'Ödeme silindi' });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
});

module.exports = router; 